...
function chats() {
  ...
  bot.on("message", async (context) => {
    ...
    if (context.message.indexOf("学习#") >= 0) return 0;
    if (context.message.indexOf("#") >= 0) {
      //判断命令，使用对应功能
      feature(context);
      return 0;
    }
    ...
    let i = 0,
      maxIndex;
    chatJson.chat.forEach(function (item) {
      //获取所有回答里的相似度并存进数组
      max_similar[i++] = Levenshtein.LevenshteinDistance.init(
        context.message.replace(/\[(\S*)]/g, ""),
        item.ask
      ).get();
    });
...
function learn() {
  ...
  bot.on("message", (context) => {
    ...
    if (context.message.indexOf("学习#") >= 0) {
      ...
      try {
        img = context.message
          .match(/\[CQ:image,file=(\S*)]/)[1]
          .replace(/,url=(\S*)term=2/, "");
        console.log(img);
        bot("get_image", {
          file: img
        })
          .then((res) => {
            console.log("下载成功");
            fs.rename(
              `${config.cqImagePath}\\${img}`,
              `${config.storagePath}\\${img}`,
              function (err) {
                if (err) {
                  console.log("移动失败");
                  throw err;
